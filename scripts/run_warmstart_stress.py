#!/usr/bin/env python3
"""Warm-start STRESS TEST: what does a WRONG prior cost?

Companion to results_warmstart_stress.txt. The benign-epoch result must survive
...
Offline, deterministic. Writes results_warmstart_stress.txt.
"""
...
def main():
    ...
    for tau in [0.15, 0.16, 0.17]:
        ...
        arms = [
            ...
            ("warm (benign)", lambda: TransferPriorUICS(prior_rates)),
            ("warm (drifted)", lambda: TransferPriorUICS(drifted)),
            ("warm (inverted)", lambda: TransferPriorUICS(inverted)),
        ]
        for name, mk in arms:
...
if __name__ == "__main__":
    ...
    print(content, end="")
    (REPO / "results_warmstart_stress.txt").write_text(content)
    print(f"\nResults written to: {REPO / 'results_warmstart_stress.txt'}")
