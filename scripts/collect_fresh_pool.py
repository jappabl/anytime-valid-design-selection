#!/usr/bin/env python3
"""Collect the FRESH-population JSON pool for the severe live test.

Identical protocol to collect_local_outcomes.py but with a NEW dataset
seed (20260812) whose 1000 prompts are verified disjoint from the
seed-42 calibration population (overlap 0), and qwen2.5:7b only. This
pool provides ground truth for a pre-registered severe live test; the
prediction window will be frozen BEFORE any temperature-0.7 sampling.
...
def main():
    ...
    for model in args.models:
        tag = MODELS[model]
        out_path = REPO / "data" / f"llm_outcomes_fresh_json_{tag}.jsonl"
        gen_path = REPO / "data" / f"fresh_json_generations_{tag}.jsonl"
        part_path = REPO / "data" / f".partial_fresh_{tag}.jsonl"
