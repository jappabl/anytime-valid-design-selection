#!/usr/bin/env python3
"""Code-pool measurement grid for the overhead law (discriminating test).

FROZEN PREDICTION (THEORY.md, before this ran): d_UI in [3,5] even
where most strata bear zero failures (d = K, not K_active);
d_single in [0.4,1.6]; d_WSR in [-0.5,0.5].
...
scripts/fit_overhead_law.py. Writes results_overhead_law_code.txt.
"""
...
if __name__ == "__main__":
    ...
    print(content, end="")
    (REPO / "results_overhead_law_code.txt").write_text(content)
    print(f"\nResults written to: {REPO / 'results_overhead_law_code.txt'}")
