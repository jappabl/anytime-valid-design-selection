#!/usr/bin/env python3
"""
Experiment B3b: CI-Based Certification Under Anytime-Valid Bounds (REVISED)

Tests whether stratified sampling improves certification outcomes when using
decision-driven stopping (certify when bounds cross threshold, not width-based).

CRITICAL FIX from initial B3b:
- Original B3b used PRECISION STOPPING (width ≤ target) → stopped too early for certification
- This version uses DECISION-DRIVEN STOPPING (UCB ≤ τ or LCB > τ) → actually attempts certification

Key differences from B2 (plug-in):
1. Decision rule: CI-based certification (UCB ≤ τ or LCB > τ), not plug-in (p̂ < τ)
2. Stopping rule: Decision-driven (stop when certified), not precision-driven (stop when narrow)
3. Budget: n_max=1000 (allows time for bounds to tighten)
4. Three outcomes: certified safe, certified unsafe, not certified (abstain at n_max)
5. Metrics: Certification rates, time-to-decision, false reject/accept, abstention rates

Design: 3×2×2 factorial
- Margin: ε ∈ {0.11, 0.13, 0.15} around τ=0.20
...
Feasibility: Script computes minimum UCB at p̂=0 to verify certification is possible.
"""
...
def run_experiment() -> List[CertificationResult]:
    ...
    print("=" * 80)
    print("EXPERIMENT B3b: CI-BASED CERTIFICATION")
    print("=" * 80)
...
def write_results_file(results: List[CertificationResult], output_path: Path):
    ...
    lines = []
    lines.append("=" * 80)
    lines.append("EXPERIMENT B3b: CI-BASED CERTIFICATION")
    lines.append("=" * 80)
    lines.append("")
...
if __name__ == "__main__":
    ...
    # Write results
    output_file = Path(__file__).parent.parent / "results_certification.txt"
