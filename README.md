...
| 7 | **Out-of-family: safety-domain test.** StrongREJECT refusal outcomes (8 local models) land in the mild-heterogeneity regime; the boundary's frozen per-pool predictions are scored either way. | In progress — see `results_safety.txt` when it lands | `data/llm_outcomes_safety_*.jsonl` |
